...
export async function initViewer(mount: HTMLElement, cfg: ViewerConfig = {}): Promise<ViewerHandle> {
  ...
  renderer.xr.setReferenceSpaceType('local-floor') // floor-aware
  ...
  // Rig (we move the rig when teleporting)
  ...
  // 🔹 Invisible teleport ground (does NOT render or occlude, but raycasts)
  const groundMat = new THREE.MeshStandardMaterial({
    color: 0x111416, transparent: true, opacity: 0
  })
  ...
  // Ambient light (HDRI will provide most lighting)
  ...
  async function loadGLB(url: string) {
    ...
    // Stop any baked animations (turntables, etc.)
    ...
    model.traverse((o: any) => {
      ...
      // Mark floors/allowed surfaces in the GLB as teleportable
  ...
  function teleportTo(point: THREE.Vector3) {
    rig.position.set(point.x, 0, point.z)      // floor-aware
  ...
  // Quest controllers (0 = right, 1 = left)
  function addController(index: number) {
    ...
    ctrl.addEventListener('select', () => {
      // Build world-space ray from controller
